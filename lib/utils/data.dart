...
class Data with ChangeNotifier{
  List<PageSort> _items = [];


  List<PageSort> get items {
    return [..._items];
  }


  void addSortingTiles(
      bool isSelected,
      String name,
      String imageUrl
      ){
    final newSortingTile = PageSort(
        isSelected: isSelected,
        name: name,
        imageUrl: imageUrl
    );
    _items.add(newSortingTile);
    notifyListeners();
  }
  ...
  Future gettingData() async {
    ...
    if (response.statusCode.toString().contains(new RegExp(r"^[2]+[0]+[0-5]$"))) {
      for(int i = 0; i<jsonData["pageSort"]["sortingTiles"].length; i++){
        addSortingTiles(
            jsonData["pageSort"]["sortingTiles"][i]["isSelected"],
            jsonData["pageSort"]["sortingTiles"][i]["name"],
            jsonData["pageSort"]["sortingTiles"][i]["imageUrl"]
        );
      }
    }
  }
